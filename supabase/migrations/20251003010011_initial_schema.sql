/*
  # Initial Database Schema for Nexus Job Platform

  1. New Tables
    - `profiles` - Core user profiles for candidates and recruiters
    ...
    - `companies` - Recruiter company profiles
      ...
      - Company details (name, logo, industry, size, website, etc.)
    
    - `job_postings` - Job listings created by recruiters
      - `id` (uuid, primary key)
      - `company_id` (uuid, references companies)
      - `recruiter_id` (uuid, references auth.users)
      - Job details (title, description, type, salary, location, etc.)
    
    - `candidate_profiles` - Extended candidate information
      - `id` (uuid, primary key)
      - `user_id` (uuid, references auth.users, unique)
      - Skills, experience, education, URLs, availability
    ...
    - `saved_jobs` - Candidates' saved job listings
    - `saved_candidates` - Recruiters' saved candidate profiles

  2. Security
    - Enable RLS on all tables
    - Profiles viewable by everyone, users can update own
    - Companies viewable by everyone, recruiters manage own
    - Job postings viewable by everyone, recruiters manage own
    - Candidate profiles viewable by everyone, candidates manage own
    - Applications visible to candidates and relevant recruiters
    - Saved items only visible to owner

  3. Triggers & Functions
    - Auto-create profile and user role on new user signup
